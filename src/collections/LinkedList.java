...
/**
 * LinkedList.java - an implementation of a linked list abstract (advanced) data
 * (dynamic) type (ADT) and useful methods, and could be "visualized" as:
 *
 * <pre>
 *             +------+       +------+       +------+       +------+
 *  null {---- |      | {---- |      | {---- |      | {---- |      |
...
public class LinkedList<T> implements Serializable {
    ...
    public boolean addFront(T data) {
        if (data == null) {
            return false;     // null data cannot be added        
        }
    ...
    public T get(int index) {
        if (!inRange(index)) {
            return null;   // invalid index, return flag        
        }
    ...
    public T removeFront() {
        if (isEmpty()) {
            return null;             // no front to remove
        }
    ...
    public boolean contains(T data) {
        if (data == null) {
            return false;         // invalid data to search for
        }
    ...
    public boolean addAfter(T data, int index) {
        if (data == null) {
            return false;              // invalid data to add
        }
        if (!inRange(index)) {
            return false;              // index out of range        
        }
        if (index == length - 1) {
            return addBack(data);    // add to end of list
        }
    ...
    public boolean addBefore(T data, int index) {
        ...
        if (index == 0) {
            return addFront(data);     // add to start of list
        }
        Node<T> node = new Node<>(data);                // create node object
        checkForLongest(node);
        Node current = getNode(index);                  // get to index spot
    ...
    /**
     * Finds the node matching the data at the first occurrence in the list and
     * returns it's index or -1 (NOT_FOUND) if not in the list
     *
    ...
    public int firstIndexOf(T data) {
        if (data == null) {
            return NOT_FOUND;     // null data rejected
        }
    ...
    /**
     * Finds the node matching the data at the last occurrence in the list and
     * returns it's index or -1 (NOT_FOUND) if not in the list
     *
     * @param data the node data to search for
    ...
    public int[] allIndices(T data) {
        if (data == null) {
            return null;       // reject null data
        }
        if (!contains(data)) {
            return null;       // no data in the list
        }
    ...
    /**
     * Deletes all occurrences of the different data items in the array from the
     * list
     *
    ...
    /**
     * Deletes all occurrences of the different data items in the passed list
    ...
    /**
     * Appends all the items from the passed list to the end of the current list
     *
    ...
    /**
     * Appends all the items from the passed list into the current list after
     * the passed index
     *
     * @param list the Linked list to append on
    ...
    /**
     * Accessor method to the encapsulated (private) property of the last (tail)
     * node of the list
     *
    ...
    protected Node getNode(int index) {
        ...
        if (index == 0) {
            return getFirstNode();   // first node returned
        }
        if (index == length - 1) {
            return getLastNode();    // last node returned
        }
    ...
    /**
     * Checks as new data is added if it has the longest "word" length, and if
     * it does, it stores that length
     *
    ...
    /**
     * Checks to see if when removing a node if it was the longest "word" length
     * node, if it was it finds the new longest "word" length node data
     *
     * @param node the Node to check the "word" length for
     */
